...
for i = 1:length(obj.Gates)
    if (obj.Gates(i).Type=="ry")  % ry q[0] = sx q[0];\nrz(_θ_0_ + pi) q[0];\nsx q[0];\nrz(3*pi) q[0]
        instr = "sx q[" + num2str(obj.Gates(i).TargetQubits -1) + "];"+newline+ "rz(" +num2str(obj.Gates(i).Angles+pi) + ...
        ") q[" + num2str(obj.Gates(i).TargetQubits-1)+ "];"+newline+ "sx q[" + num2str(obj.Gates(i).TargetQubits-1) + ...
        "];"+newline+"rz(3*pi) q[" + num2str(obj.Gates(i).TargetQubits-1)+"];"+newline;
    else
